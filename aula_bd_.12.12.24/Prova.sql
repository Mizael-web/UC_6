...
CREATE TABLE IF NOT EXISTS servicos(
  ...
  preco decimal (10,2) NOT NULL CHECK ( PRECO > 0),
	tipo_servico VARCHAR (80) NOT NULL CHECK (tipo_servico IN ('CONSULTORIA', 'SUPORTE', 'MANUTENCAO'))   
);
...
CREATE TABLE IF NOT EXISTS chamadas (
    ...
    data_cadastro DATE NOT NULL,
  	status VARCHAR(50) NOT NULL CHECK ( STATUS IN ('PENDENTE', 'EM ANDAMENTO', ' FINALIZADO')),
    descricao text NOT NULL	
);  
...
INSERT INTO chamadas (clientes_id, tecnicos_id, servicos_id, data_cadastro, status, descricao)
VALUES
...
SELECT clientes.nome, chamadas.descricao
FROM Clientes
JOIN Chamadas ON Clientes.id = Chamadas.clientes_id
WHERE Chamadas.status = 'EM ANDAMENTO';


-- 10. RECUPERAR OS NOMES DOS TÉCNICOS QUE REALIZARAM SERVIÇOS COM O TIPO "MANUTENÇÃO"
SELECT DISTINCT tecnicos.nome, 
FROM tecnicos
WHERE tipo_servico = 'MANUTENCAO';

-- 11. MOSTRAR NOME DOS CLIENTES E O VALOR PAGO
SELECT clientes.nome, SUM(valor_pago) AS valor_pago 
FROM clientes 
JOIN pagamento  ON CLIENTES.id = pagamento.cliente_id
GROUP BY clientes.nome;


-- 12. LISTAR CLIENTES E SERVIÇOS E A DESCRIÇÃO
SELECT clientes.nome, tipo_servicos, servicos.descricao, servicos.nome, servicos.preco
FROM clientes
...
UPDATE servicos 
SET preco = 350
WHERE tipo_servicos = 'MANUTENCAO' AND preco < 350;

--15. EXCLUA OS TÉCNICOS QUE NÃO TÊM CHAMADOS REGISTRADOS.

DELETE *FROM tecnicos
WHERE id NOT IN (
    SELECT DISTINCT tecnicos_id 
    FROM chamadas
);
